...
use crate::talk_to_vitalik::{get_on_chain_deal_info, post_proof};
...
impl ProofScheduleDb {
    fn schedule(&self, block_num: BlockNum, deal_id: DealID) -> Result<()> {
    ...
    async fn add_a_deal_to_db(&self, deal_id: DealID, _cid: Cid) -> Result<()> {
    ...
    async fn wake_up(&self) -> Result<()> {
        ...
        for block_and_deals in self.schedule_tree.range(current_through_block_n..current_block_n) {
            ...
            for deal_id in deal_ids.iter() {
                ...
                let deal_params = self.deal_tree.get(deal_id)?.ok_or_else(|| anyhow!("no deal params found for deal id {:?}", deal_id))?;
                let proof_to_post = gen_proof(block, deal_params.on_chain_deal_info.ipfs_file_cid, deal_params.on_chain_deal_info.ipfs_file_size).await?;
                post_proof(deal_id, proof_to_post).await?;
            }
